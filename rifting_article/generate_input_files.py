"""
This script creates the input files required by Mandyoc code to run the preliminary scenario and the rifting experiments.

1) Select the kind of experiment in the variable 'experiment' and set the respective parameters.
    1.1) For preliminary scenario you do not need to set any parameter, just select the experiment.
    
    1.2) For rifting experiments you have to:
        - Set the scale factor for the effective viscosity of the lower crust in lower_crust_effective_viscosity_scale_factor variable;
        - Choose if the lithosphere is homogeneous or thinned in the variable homogeneous_mlit:
            - True for homogeneous lithosphere
            - False for thinned lithosphere;
        - Set the extension velocity in the variable velocity (in cm/yr);
"""
...
if(experiment == 'rifting'):
    ...
    DeltaT = 230 # oC increase in basal temperature
...
if(experiment == 'rifting' or experiment == 'preliminary'):
    if(seed_in_litho):
        # seed depth bellow base of lower crust (m)
        seed_depth = 6 * 1.0e3 #9 * 1.0e3 #original
        thickness_seed = 12 * 1.0e3
    ...
    if(experiment != 'preliminary' and homogeneous_mlit==False): #Building a ramp in the lithosphere to represent a smooth transition from 120 km to 80 km
...
if(preset == False):
    T = 1300 * (z - thickness_air) / (thickness_lithospherespheric_mantle)  # Temperature of 1300 isotherm bellow the lithosphere
...
if(preset == False):
    ...
    scenario_infos.append(f'\tPath to external scenario used for build initial temperature field: {scenario}')
    if(from_dataset):
...
if(preset == False):
    ...
    print(f'\tPath to external scenario used for build initial temperature field: {scenario}')
