...
def plot_tracked_particles_depth_coded(trackdataset, ax, i, hcrust=35.0e3, markersize=4,
    ...
    z_track = trackdataset.ztrack.values[::-1] #the values of z are negative, so we need to multiply by -1 to plot them in the correct position
...
def plot_ptt_paths_depth_coded(trackdataset, ax, instants=[], scale_correction = 1.0e-3, hcrust=35.0e3, alpha_lines=1.0, alpha_points=1.0, plot_lower_crust_particles=False, plot_mantle_lithosphere_particles=True, plot_asthenosphere_particles=True, color_lower_crust='xkcd:brown', color_mlit_upper='xkcd:cerulean blue', color_mlit_intermediate='xkcd:scarlet', color_mlit_lower='xkcd:dark green'):
    ...
    for particle, particle_layer in zip(range(n), particles_layers):
        ...
        if((plot_lower_crust_particles == True) & (particle_layer == lower_crust_code)):
            if(cond_lower_crust2plot[particle] == True):
                ax.plot(T[::, particle], P[::, particle]*scale_correction, '-', color=color_crust, linewidth=linewidth, alpha=alpha_lines, zorder=60) #PTt path
                
                if(len(instants)>0):
                    for instant in instants:
                        idx = find_nearest(time, instant)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_lower_crust, markersize=markersize, alpha=alpha_points, zorder=60)
                else: #plotting points at each 5 Myr
                    for j in np.arange(0, time[-1], 5):
                        idx = find_nearest(time, j)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_lower_crust, markersize=2, alpha=alpha_points, zorder=60)


        if((plot_mantle_lithosphere_particles == True) & ((particle_layer == mantle_lithosphere1_code) | (particle_layer == mantle_lithosphere2_code))): #lithospheric mantle particles
            if(cond_upper_2plot_mlit[particle] == True):
                ax.plot(T[::, particle], P[::, particle]*scale_correction, '-', color=color_mlit_upper, linewidth=linewidth, alpha=alpha_lines, zorder=61) #PTt path
                if(len(instants)>0):
                    for instant in instants:
                        idx = find_nearest(time, instant)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_mlit_upper, markersize=markersize, alpha=alpha_points, zorder=60)
                else: #plotting points at each 5 Myr
                    for j in np.arange(0, time[-1], 5):
                        idx = find_nearest(time, j)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color='xkcd:black', markersize=markersize-2, alpha=alpha_points, zorder=60)
            elif(cond_intermediate_2plot_mlit[particle] == True):
                ax.plot(T[::, particle], P[::, particle]*scale_correction, '-', color=color_mlit_intermediate, linewidth=linewidth, alpha=alpha_lines, zorder=61)
                if(len(instants)>0):
                    for instant in instants:
                        idx = find_nearest(time, instant)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_mlit_intermediate, markersize=markersize, alpha=alpha_points, zorder=60)
                else: #plotting points at each 5 Myr
                    for j in np.arange(0, time[-1], 5): 
                        idx = find_nearest(time, j)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color='xkcd:black', markersize=markersize-2, alpha=alpha_points, zorder=60)
            elif(cond_lower_2plot_mlit[particle] == True):
                ax.plot(T[::, particle], P[::, particle]*scale_correction, '-', color=color_mlit_lower, linewidth=linewidth, alpha=alpha_lines, zorder=61)
                if(len(instants)>0):
                    for instant in instants:
                        idx = find_nearest(time, instant)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_mlit_lower, markersize=markersize, alpha=alpha_points, zorder=60)
                else: #plotting points at each 5 Myr
                    for j in np.arange(0, time[-1], 5):
                        idx = find_nearest(time, j)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color='xkcd:black', markersize=markersize-2, alpha=alpha_points, zorder=60)
        ...
        if((plot_asthenosphere_particles == True) & (particle_layer == asthenosphere_code)):
            if(cond_upper_2plot_ast[particle] == True):
                ax.plot(T[::, particle], P[::, particle]*scale_correction, '-', color=color_mlit_upper, linewidth=linewidth-1, alpha=alpha_lines, zorder=61)
                if(len(instants)>0):
                    for instant in instants:
                        idx = find_nearest(time, instant)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_mlit_upper, markersize=4, alpha=alpha_points, zorder=60)
                else: #plotting points at each 5 Myr
                    for j in np.arange(0, time[-1], 5):
                        idx = find_nearest(time, j)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color='xkcd:black', markersize=0.7, alpha=alpha_points, zorder=60)
            elif(cond_intermediate_2plot_ast[particle] == True):
                ax.plot(T[::, particle], P[::, particle]*scale_correction, '-', color=color_mlit_intermediate, linewidth=linewidth-1, alpha=alpha_lines, zorder=61)
                if(len(instants)>0):
                    for instant in instants:
                        idx = find_nearest(time, instant)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_mlit_intermediate, markersize=4, alpha=alpha_points, zorder=60)
                else: #plotting points at each 5 Myr
                    for j in np.arange(0, time[-1], 5):
                        idx = find_nearest(time, j)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color='xkcd:black', markersize=0.7, alpha=alpha_points, zorder=60)
            elif(cond_lower_2plot_ast[particle] == True):
                ax.plot(T[::, particle], P[::, particle]*scale_correction, '-', color=color_mlit_lower, linewidth=linewidth-1, alpha=alpha_lines, zorder=61)
                if(len(instants)>0):    
                    for instant in instants:
                        idx = find_nearest(time, instant)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color=color_mlit_lower, markersize=4, alpha=alpha_points, zorder=60)
                else: #plotting points at each 5 Myr
                    for j in np.arange(0, time[-1], 5):
                        idx = find_nearest(time, j)
                        ax.plot(T[idx, particle], P[idx, particle]*scale_correction, '.', color='xkcd:black', markersize=0.7, alpha=alpha_points, zorder=60)

def plot_ptt_paths_depth_coded_frame(trackdataset, ax, i, current_time, hcrust=35.0e3, alpha_lines=1.0, markersize=6, linewidth=0.35,
                                     plot_lower_crust_particles=False, plot_mantle_lithosphere_particles=True, plot_asthenosphere_particles=True,
                                     color_lower_crust='xkcd:brown',
